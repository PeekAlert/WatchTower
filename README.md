...
- **Re-run All** — Re-run every audited member at once from the dashboard toolbar, with a live per-member progress list; capped to 3 audits running at a time (the next queued member starts as each one finishes) so a large fleet's config parsing, port scans, and BGP lookups don't all compete for resources at once
...
- **Custom topology upload** — An admin can upload a topology diagram exported from another tool (standalone HTML) on a member's Info tab. Once uploaded, it replaces the auto-discovered diagram as what the dashboard's Topology button opens, and can be removed at any time to fall back to the built-in diagram. Run Audit's **Generate Topology** / **Use saved topology** / **Import Custom Topology HTML** options are mutually exclusive — checking one clears the other two, so a run can't be asked to both rebuild from scratch and reuse an existing diagram. Viewed live, it opens through a dedicated route so the diagram's own external resources load normally; a downloaded copy of the report keeps its topology map working with no server or login needed
...
- **Full session logging** — Every session's complete transcript is recorded and viewable later, with SSH/enable passwords redacted from the stored log. Reviewable three ways: a **History** button on each device row (no need to open a new session first), a member's **Session History** tab covering every device on that member, and — for admins — a platform-wide **Session History** page in the main nav covering every member at once, filterable by member. Each entry is labeled when it was a tenant user's session; sessions with an empty transcript (failed connect, closed instantly) are left out everywhere. Viewing history is its own permission (`Can view terminal session history`, Users → Edit), independent of being able to open a new live session
- **Team-wide visibility for tenants** — A non-admin viewing Session History sees every session on a member they're assigned to (not just their own) — there's no need to track down which teammate ran a given command
...
- **Choice of navigation layout** — Switch between a horizontal top nav bar or the classic left sidebar from a dropdown next to the theme picker, on every page; a per-browser preference like the theme, switch back any time with no page reload
