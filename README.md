...
A multi-user web platform for running, scheduling, and reporting on network device security audits. Analyzes Cisco IOS, NX-OS, JunOS, Arista EOS, and other device configuration files for security misconfigurations, scores each member network, and delivers alerts and reports automatically.
...
- **Location map** — Address geocoding with an embedded map on the member page and in exported reports, rendered from free OpenStreetMap data rather than a third-party embed that requires its own API key or account
...
- **Web-based self-upgrade** — Upload a new release tarball in Settings to upgrade in place, with an automatic pre-upgrade backup and a live progress log; database, member configs, and reports are never touched
- **Check for updates** — Settings checks this releases page on load and shows an "Upgrade available" banner with the new version number when one exists, with a one-click Download & Upgrade button that runs the same safe upgrade pipeline as a manual upload
...
> The tag/tar filename doesn't indicate the OS it'll run on beyond the architecture (`x86_64`/Linux) — the binary bundle includes its own Python runtime and the templates/scan profiles/public license key needed to run standalone.
...
| Checks | Enable/disable individual audit checks — score updates immediately |
...
This option only works for releases built after `upgrade.sh` was added to the release bundle (`v1.0.8` and later) — older tarballs will be rejected by the upload validation.
...
Run `uninstall.sh` from **inside the install directory you want to remove** — it checks for `run` in its own directory and deletes that directory, so it has to actually live there, not in some other/temporary location:
...
**Uninstalling an older install that predates `uninstall.sh`** (releases before v1.2.3): grab the tarball URL for the latest release from the **Releases** page (`Assets → watchtower-vX.Y.Z-linux-x86_64.tar.gz`), then copy just the script into your existing install directory rather than extracting the whole tarball over it:

```bash
curl -L -o watchtower.tar.gz <asset URL from the Releases page>
tar xzf watchtower.tar.gz
cp network-audit/uninstall.sh /path/to/your/existing/install/
